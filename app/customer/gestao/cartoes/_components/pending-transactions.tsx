...
export function PendingTransactions() {
  ...
  const loadTransactions = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/client-management/card-sales?status=pending');
      if (!response.ok) throw new Error('Erro ao carregar transações');
      
      const data = await response.json();
      console.log("[PENDING_TRANSACTIONS] Dados recebidos:", data);
  ...
  const handleConfirmReceipts = async () => {
    ...
    // Funcionalidade de confirmação de recebimentos ainda não implementada
    toast.info('Funcionalidade de confirmação de recebimento em desenvolvimento');
    setShowConfirmDialog(false);
    
    // try {
    //   setConfirming(true);
    //   const response = await fetch('/api/client-management/card-sales/confirm-batch', {
    //     method: 'POST',
