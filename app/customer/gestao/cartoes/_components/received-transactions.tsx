...
export function ReceivedTransactions() {
  ...
  const loadTransactions = async (filters?: { startDate?: string; endDate?: string }) => {
    try {
      ...
      const response = await fetch(`/api/client-management/card-sales?${params}`);
      if (!response.ok) throw new Error('Erro ao carregar transações');
      
      const data = await response.json();
      console.log("[RECEIVED_TRANSACTIONS] Dados recebidos:", data);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                    ...
                    <div className="text-sm space-y-1">
                      ...
                      <div className="text-gray-500 text-xs">
                        {transaction.paymentMethod === 'DEBIT_CARD' ? 'Cartão de Débito' : 'Cartão de Crédito'}
                      </div>
                    </div>
                  </div>
                </div>
