...
export default function InsumosPage() {
  ...
  const handleSave = async () => {
    try {
      ...
      const url = editingSupply
        ? `/api/admin/pricing/supplies/${editingSupply.id}`
        : '/api/admin/pricing/supplies'
  ...
  const handleDelete = async (supply: GlobalSupply) => {
    ...
    try {
      const response = await fetch(`/api/admin/pricing/supplies/${supply.id}`, {
        method: 'DELETE'
      })
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold flex items-center gap-3">
                ...
                Catálogo de Insumos de Produção
              </h1>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-6 text-sm text-gray-600">
                          ...
                          {supply.SupplyRecipe && (
                            <div>
                              <span className="font-medium text-orange-600">Custo calculado: </span>
                              <span className="text-orange-700 font-semibold">
                                {formatCurrency(supply.SupplyRecipe.estimatedCost / supply.SupplyRecipe.yieldAmount)}/{supply.SupplyRecipe.yieldUnit}
                              </span>
                            </div>
                          )}
