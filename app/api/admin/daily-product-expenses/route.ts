...
export async function GET(request: Request) {
  try {
    ...
    const allProductExpenses = await prisma.expense.findMany({
      where: { expenseType: 'PRODUCTS' },
      include: {
        Category: true,
    ...
    const normalExpenses = allProductExpenses.filter((exp: any) => {
    ...
    // Combinar e formatar as despesas
    const allExpenses = [
      ...normalExpenses.map((exp: any) => ({
        id: exp.id,
      ...
      ...creditCardExpenses.map((exp: any) => ({
        ...
        description: exp.description,
        amount: exp.amount,
        status: 'PENDING', // Cartão sempre pendente até a fatura
    ...
    console.error('[daily-product-expenses] Erro:', error)
