...
export async function PUT(
  ...
  try {
    ...
    const oldExpense = await prisma.clientExpense.findUnique({
      where: { id: params.id },
    });
    ...
    // Se mudou de PENDING para PAID, criar transação
    if (oldExpense?.status !== "PAID" && body.status === "PAID" && body.bankAccountId) {
    ...
    console.error("[CLIENT_EXPENSE_PUT] Error:", error);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.clientExpense.delete({
      where: {
        id: params.id,
        customerId,
      },
    ...
    console.error("[CLIENT_EXPENSE_DELETE] Error:", error);
    return NextResponse.json(
